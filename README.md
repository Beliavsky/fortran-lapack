...
This repository is a downstream fork of [Federico Perini's fortran-lapack](https://github.com/perazz/fortran-lapack). It extends `pure` declarations through additional BLAS/LAPACK procedures and interfaces.
The purity analysis and refactoring was done by GPT-5.6 Sol.

- The [pure-procedure comparison report](PURE_PROCEDURE_DIFFERENCES.md), generated by [the comparison script](scripts/compare_pure_procedures.py), provides commit-pinned source comparisons with the upstream repository.
- The [remaining non-pure procedure report](IMPURE_PROCEDURES.md), generated by [the audit script](scripts/list_impure_procedures.py), inventories the source procedures that still require purity review.
- The [pure API alternatives report](PURE_API_ALTERNATIVES.md) maps impure compatibility functions to pure functions, operators, and subroutines.
...
```
[dependencies]
fortran-lapack = { git="https://github.com/Beliavsky/fortran-lapack.git" }
```
...
```
[dependencies]
fortran-lapack = { git="https://github.com/Beliavsky/fortran-lapack.git", preprocess.cpp.macros=["LA_EXTERNAL_BLAS", "LA_EXTERNAL_LAPACK"] }
```
