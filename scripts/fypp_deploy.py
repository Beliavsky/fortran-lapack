...
PROCEDURE_DECLARATION = re.compile(
    r"^(?P<indent>\s*)(?P<prefix>.*?)(?P<kind>subroutine|function)\s+"
    r"(?P<name>[a-z][a-z0-9_]*)\b",
    re.IGNORECASE,
)


def load_pure_procedures():
    """Return procedure names whose generated declarations must remain pure."""
    path = os.path.join(ROOT, "scripts", "pure_procedures.txt")
...
PURE_PROCEDURES = load_pure_procedures()
...
def apply_pure_manifest(path):
    """Apply curated PURE attributes to generated procedure declarations."""
    with open(path, encoding="utf-8") as fid:
        lines = fid.readlines()
    changed = False
    for index, line in enumerate(lines):
        match = PROCEDURE_DECLARATION.match(line)
        if match is None or match.group("name").lower() not in PURE_PROCEDURES:
            continue
        prefix = match.group("prefix")
        if re.search(r"\b(end|procedure|pure|elemental|impure)\b", prefix, re.IGNORECASE):
            continue
        start = match.start("prefix")
        lines[index] = line[:start] + "pure " + line[start:]
        changed = True
    if changed:
        with open(path, "w", encoding="utf-8", newline="") as fid:
